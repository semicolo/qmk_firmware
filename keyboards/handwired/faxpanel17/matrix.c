...
uint8_t matrix_scan(void){
  ...
  matrix_scan_quantum();
  if (matrix_get_row(0))
    write259(LEDON, 0); // send
  else
    write259(LEDOFF, 0); // send
  if (matrix_get_row(1))
    write259(LEDON, 1); // send
  else
    write259(LEDOFF, 1); // send

  if (matrix_get_row(2))
    write259(LEDON, 2); // send
  else
    write259(LEDOFF, 2); // send
...
static matrix_row_t read_cols(void){
    ...
    palClearPad(GPIOA, 1);
   matrix_row_t data =
    ((palReadPad(GPIOB, 15)==PAL_HIGH) ? 0 : (1<<10))
    | ((palReadPad(GPIOB, 14)==PAL_HIGH) ? 0 : (1<<9))
    | ((palReadPad(GPIOB, 13)==PAL_HIGH) ? 0 : (1<<8))
    | ((palReadPad(GPIOB, 12)==PAL_HIGH) ? 0 : (1<<7))
    | ((palReadPad(GPIOB, 11)==PAL_HIGH) ? 0 : (1<<6))
    | ((palReadPad(GPIOB, 10)==PAL_HIGH) ? 0 : (1<<5))
    | ((palReadPad(GPIOB, 9)==PAL_HIGH) ? 0 : (1<<4))
